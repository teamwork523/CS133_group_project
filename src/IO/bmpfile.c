...
// Return a bmpfile_t if success, or NULL if error occurs.
// If file is not 8-bpp (depth = 8), NULL will be returned.
bmpfile_t *bmp_create_8bpp_from_file(const char *filename)
{
  ...
  bmp_read_header(result, fp);
  bmp_read_dib(result, fp);

  if (result->dib.depth != 8) {
    free(result);
    return NULL;
  }
  ...
  // representation.
  // You should add your own data structure to bmpfile_t to
  // store raw data for color index, which helps you avoid
  // manipulating RGB pixels.
  bmp_8bpp_read_pixels(result, fp);
...
{
  if (bmp == NULL || (x >= bmp->dib.width) || (y >= bmp->dib.height))
    return NULL;
...
{
  if (bmp == NULL || (x >= bmp->dib.width) || (y >= bmp->dib.height)) {
    // fprintf(stderr, "%u * %u; (x = %u, y = %u)\n",
    //         bmp->dib.width, bmp->dib.height, x, y);
    return FALSE;
  }
...
// Takes a pixel in rgb_pixel_t and returns the color index that
// is identical or the most similar to it.
int
...
// Takes a color index from an 8-bpp BMP and converts it back to rgb_pixel_t.
rgb_pixel_t*
get_8bpp_color(bmpfile_t *bmp, const unsigned char color_index) {
  if (bmp == NULL) return NULL;
  return &(bmp->colors[color_index]);
}
...
bmp_8bpp_read_pixels(bmpfile_t *bmp, FILE *fp) {
  ...
  for (pixel_index = 0; pixel_index < total_pixels; ++pixel_index) {
    ...
    const rgb_pixel_t *pixel = get_8bpp_color(bmp, color_index);
    x = pixel_index % width;
...
bool bmp_cmp(const bmpfile_t *bmp1, const bmpfile_t *bmp2) {
  if (bmp1 == NULL || bmp2 == NULL) {
    return FALSE;
  }
  return (memcmp(bmp1, bmp2, sizeof(bmpfile_t)) == 0) ? TRUE : FALSE;
}
